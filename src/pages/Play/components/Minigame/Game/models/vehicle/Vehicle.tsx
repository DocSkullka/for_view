...
export function Vehicle({ angularVelocity, children, position, rotation }: VehicleProps) {
  ...
  const wheelInfos = wheels.map((wheelGroup, index): DerivedWheelInfo => {
    if (wheelGroup.current) {
      wheelGroup.current.children[0].rotation.z = 1.57;
    }
  ...
  useFrame((state, delta) => {
    ...
    if (!editor) {
      if (camera === 'FIRST_PERSON') {
        ...
        v.set((Math.sin(steeringValue) * speed) / 2.5, 2.55 + (engineValue / 1000) * -0.5, -3.5 - speed / 15 + (controls.brake ? 1 : 0))
      }
  ...
  // Camera sway
  swaySpeed = isBoosting ? 40 : 20; // Уменьшена скорость колебаний
  swayTarget = isBoosting ? (speed / maxSpeed) * 4 : (speed / maxSpeed) * 1; // Уменьшена амплитуда колебаний
  swayValue = isBoosting ? (speed / maxSpeed + 0.25) * 15 : MathUtils.lerp(swayValue, swayTarget, delta * (isBoosting ? 8 : 16)); // Уменьшено влияние скорости
  defaultCamera.rotation.z += (Math.sin(state.clock.elapsedTime * swaySpeed * 0.9) / 2000) * swayValue; // Уменьшено влияние на вращение
  defaultCamera.rotation.x += (Math.sin(state.clock.elapsedTime * swaySpeed) / 2000) * swayValue; // Уменьшено влияние на вращение

  // Vibrations
  chassisBody.current!.children[0].rotation.x = (Math.sin(state.clock.getElapsedTime() * 20) * (speed / maxSpeed)) / 200; // Уменьшены вибрации
  chassisBody.current!.children[0].rotation.z = (Math.cos(state.clock.getElapsedTime() * 20) * (speed / maxSpeed)) / 200; // Уменьшены вибрации
});
